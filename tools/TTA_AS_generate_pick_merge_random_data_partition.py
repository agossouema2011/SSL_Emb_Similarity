...
def generate_pick_merge_random(random_file, random_percent, indicator_file, pick_percent, save_file, static_file, reverse=True):
    ...
    # create a new datasets/coco/annotations/coco_training_e_new.json
    ...
    with open("datasets/coco/annotations/coco_training_e_new.json", "w") as data2:
    ...
    with open('datasets/coco/annotations/coco_training_e_new.json') as f:
    ...
    for jj in range(len(tableImages)):
       ...
       # get from  coco_training_new.json the "id" value for "imageName"
        ...
        for iii in range(len(bbox_predicted)):
            ...
            elt_for_csv=imageName + "," + str(pred_class_predicted[iii])+ "," + str(bbox_predicted_tab[0])+ "," + str(bbox_predicted_tab[1]) +","+ str(bbox_predicted_tab[2]) + "," + str(bbox_predicted_tab[3]) + "," + str(the_area)
    ...
    with open("datasets/coco/annotations/coco_training_e_new.json",'w') as f:
    ...
    with open('./results/e_coco/TTA_AS.csv', 'w') as f:
