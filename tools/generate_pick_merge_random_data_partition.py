...
def generate_pick_merge_random(random_file, random_percent, indicator_file, pick_percent, save_file, static_file, reverse=True):
    ...
    # create a new datasets/coco/annotations/coco_training_c_new.json
    ...
    with open("datasets/coco/annotations/coco_training_c_new.json", "w") as data2:
    ...
    with open('datasets/coco/annotations/coco_training_c_new.json') as f:
    ...
    csv_tab=[] # to store data for csv file
    ...
    for jj in range(len(tableImages)):
        ...
        for kk in predictionList:
            ...
            pred_class_predicted.append(kk.get("pred class"))# the predicted class for the selected image
            TThe_area=(kk.get("pred box")[2] - kk.get("pred box")[0]) * (kk.get("pred box")[3] - kk.get("pred box")[1])
            elt_for_csv=imageName + "," + str(kk.get("pred class"))+ "," + str(kk.get("pred box")[0])+ "," + str(kk.get("pred box")[1]) +","+ str(kk.get("pred box")[2]) + "," + str(kk.get("pred box")[3]) + "," + str(TThe_area)
            csv_tab.append(elt_for_csv)
       ...
       # get from  coco_training_new.json the "id" value for "imageName"
        ...
        for iii in range(len(bbox_predicted)):
            the_area=(bbox_predicted[iii][2] - bbox_predicted[iii][0]) * (bbox_predicted[iii][3] - bbox_predicted[iii][1])
            id_new=id_new+1
            tab_elt={"iscrowd": 0, "ignore": 0, "image_id": idImage, "bbox": bbox_predicted[iii], "area": the_area, "segmentation": [], "category_id": pred_class_predicted[iii], "id": id_new}
    ...
    with open("datasets/coco/annotations/coco_training_c_new.json",'w') as f:
        f.write(json.dumps(new_data_annotation))
            
    #------------------------------------------ Initializing the codes for creating CSV file --------------------------
    print("Nb Lines csv_tab=",len(csv_tab))
    header = ['file_name', 'PredL', 'x1', 'y1', 'x2', 'y2', 'area']
    with open('./results/c_coco/AS.csv', 'w') as f:
        writer = csv.writer(f, delimiter=',')
        # write the header
        writer.writerow(header)
        for elt_i in range(len(csv_tab)):
            #print("elt_i=",csv_tab[elt_i],"\n")
            writer.writerow([csv_tab[elt_i]])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--indicator_file",type=str,default='results/c_coco/10random_maxnorm.txt')
    parser.add_argument("--pick_percent",type=float,default=10.0)
    parser.add_argument("--reverse",type=bool,default=True)
    parser.add_argument("--save_file",type=str,default='dataseed/c_coco_pick/pick_maxnorm10+random10.txt')
    parser.add_argument("--static_file",type=str,default='temp/coco/static_by_random.json/c_static_by_random.json')
