...
def generate_pick_merge_random(random_file, random_percent, indicator_file, pick_percent, save_file, static_file, cfg,reverse=True):
    ...
    with open(random_file,'r') as f:
        ...
        for i in range(10):
            ...
            iddx = []
            for item in idx:
                if item not in exist_idx:  # select from the inference "results/i_coco/random_maxnorm.json" only the results where image is not in the "dataseed/COCO_supervision.txt "
                    iddx.append(item)
    ...
    #--------------------------- UPDATE dataset annotations-----------------------------------
    # create a new datasets/coco/annotations/coco_training_i_new.json
    # First make coco_training_new.json as a copy of datasets/coco/annotations/coco_training.json
    with open("datasets/coco/annotations/coco_training.json", "r") as data1:
        data1content=json.load(data1)
        
    with open("datasets/coco/annotations/coco_training_i_new.json", "w") as data2:
    ...
    with open('datasets/coco/annotations/coco_training_i_new.json') as f:
    ...
    with open("datasets/coco/annotations/coco_training_i_new.json",'w') as f:
